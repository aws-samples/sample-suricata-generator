...
**Latest feature — v2.8.3:** **Multi-Rule-Group Analysis** — review your whole firewall policy at once by analyzing several stateful rule groups together, not just the one open in the editor. See [Rule Group Analysis](#rule-group-analysis).
...
> 💡 **TL;DR:** Install Python 3.7+ with tkinter, then run `python3 suricata_generator.py`. Per-OS commands are in the collapsible sections below.

### System Requirements

**Required:** Python 3.7+ and tkinter (GUI library)
**Optional:** wxPython (only for the [Advanced Editor](#advanced-editor-new-in-v1190-enhanced-in-v1230) with code folding)
...
> 💡 All core features work without wxPython — it is only needed for the [Advanced Editor](#advanced-editor-new-in-v1190-enhanced-in-v1230).

<details>
<summary><b>Install wxPython</b></summary>

```bash
pip install wxPython   # or pip3 on some systems
...
> 💡 See **Help > AWS Setup** in the app for a guided setup (Prerequisites, IAM, Credentials, and a built-in connection test). Including AWS managed rule groups also needs `network-firewall:ListRuleGroups` and `network-firewall:DescribeRuleGroup`.
...
1. Install dependencies: `pip install boto3 intervaltree requests`
