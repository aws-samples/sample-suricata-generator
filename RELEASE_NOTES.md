...
- **Graceful Startup Without Optional Dependencies**: Fixed a crash on startup when `boto3` is not installed. The AI Rule Assistant's import chain (`nl_parser.py`) now guards the `boto3` import so the application launches normally without it. Users see a friendly popup when they try to access AI features without boto3 installed.
- **Graceful Palo Alto Import Without defusedxml**: The `defusedxml` dependency for the Palo Alto configuration importer is now optional at startup. The application launches normally without it and shows a "Missing Dependency" popup with install instructions when users attempt a Palo Alto import without `defusedxml` installed.

---

## Version 2.2.1 - April 23, 2026

### Bug Fixes

- **macOS Dark Mode Support**: Fixed unreadable text in the AI Rule Assistant chat panel and main application Treeview widgets when macOS dark mode is enabled. The UI now detects the system theme and applies appropriate color palettes for both light and dark modes.
- **XML Security Hardening**: Replaced the standard library XML parser with `defusedxml` in the Palo Alto configuration importer to protect against XML External Entity (XXE) and entity expansion attacks in imported firewall configuration files.
- **AI Model Default Selection**: The AI Rule Assistant now defaults to Claude Opus 4 (the latest flagship model) instead of selecting the first available Claude model alphabetically.
- **README Disclaimer**: Added a disclaimer to the README clarifying that all rules generated by this tool are the customer's responsibility to vet, test, and validate before use.

---

## Version 2.2.0 - April 20, 2026

### New Feature: AI Rule Assistant

Generate AWS Network Firewall Suricata rules from plain English descriptions using Amazon Bedrock. Accessible from **Tools > AI Rule Assistant**, the **✨ Insert AI Rule** button in the editor toolbar, or **Ctrl+I**.

#### Key Capabilities
- **Natural Language Rule Generation**: Describe traffic to detect or block in plain English — the AI generates valid, AWS Network Firewall-compatible Suricata rules
- **Multi-Layer Validation Pipeline**: Generated rules pass through 34 auto-fix patches, syntax validation, best practice checks, and AWS-specific guardrails (8,192 char limit, protocol/category compatibility, SID uniqueness)
- **Chat Interface**: Conversational UI with chat history, model/region selection, and the ability to ask follow-up questions about Suricata rules
- **Plain-English Summaries**: Each generated rule includes a human-readable summary explaining what it does (e.g., "Block (with reset) tls traffic matching .ru")
- **Example Prompts**: Randomly selected clickable example prompts help new users get started
- **One-Click Insertion**: Insert generated rules directly into the rule list with proper SID assignment matching the main editor's sequence
- **Change Tracking Integration**: AI-inserted rules create baseline snapshots for revision history, consistent with all other insertion methods
- **Session Disclaimer**: Informational disclaimer displayed once per session reminding users to test rules before production deployment

#### AWS Setup
- Requires Amazon Bedrock model access — see **Help > AWS Setup** for IAM permissions and setup instructions
- Supported regions: us-east-1, us-west-2, eu-west-1, ap-northeast-1, ap-southeast-1
- IAM policy updated with `bedrock:InvokeModel`, `bedrock:ListFoundationModels`, `bedrock:ListInferenceProfiles`
- Help > AWS Setup > Testing tab now verifies Bedrock connectivity and model access

#### Data File Enrichments
- Added complete AWS URL/Domain Category list (52 categories) to grounding data
- Added HTTP/2 domain filtering patterns from official AWS documentation
- Added rule evaluation order guidance (strict vs. default action order)
- Added domain list auto-generated rule patterns
- Added SNI allowlist with certificate issuer verification (xbits) patterns
- Added `http.request_header` and `http2.header_name` keywords
- Updated actions to reflect AWS Network Firewall's 4 supported actions (removed rejectsrc/rejectdst/rejectboth)
